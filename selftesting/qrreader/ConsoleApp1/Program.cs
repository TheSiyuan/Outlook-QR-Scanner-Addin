...
namespace ConsoleApp1
{
    class Program
    {
        static string QRImgPath = @"E:\VS Project\GitHub\QR Code Things\selftesting\test subject";
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                //存放结果的文件
                string filePath = "txtResult" + DateTime.Now.ToString("yyyyMMddHHmmssfff") + ".VCF";
                
                //一个个读取并追加到记录文件
                for (int i = 0; i < files.Length; i++)
                {
                    File.AppendAllText(filePath, "\n" + CodeDecoder(files[i]) + "\t" + files[i] + "\n");//追加到文件里记录
